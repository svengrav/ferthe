...
/**
 * Fetches nearby POIs from OSM based on user preferences and returns
 * them as StumbleSuggestions for the client to display on the map.
 */
export function createStumbleApplication(): StumbleApplicationActions {
  const getSuggestions = async (
    ...
    try {
      ...
      const pois = await osmConnector.fetchPois(lat, lon, radius, preferences)

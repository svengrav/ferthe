...
export function MapEditor() {
  ...
  const [selectedItem, setSelectedItem] = useState<
    { type: "spot" | "trail"; id: string } | null
  >(null);
        ...
        <div className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center space-x-2">
                <input
                  ...
                  className="rounded accent-[#ec4899]"
                />
          ...
          <div className="border-t border-gray-300 pt-4">
            ...
            <div className="space-y-2">
              <button
                type="button"
                className="w-full px-4 py-2 text-sm bg-gray-950 rounded hover:bg-gray-700"
              >
                Create Spot
              </button>
              <button
                type="button"
                className="w-full px-4 py-2 text-sm bg-purple-600 rounded hover:bg-purple-700"
              >
                Create Trail
              </button>
            </div>
              ...
              <div className="space-y-1">
                {spots.map((spot) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setSelectedItem({ type: "spot", id: spot.id })}
                    className={`w-full text-left px-3 py-2 rounded text-sm ${
                      selectedItem?.type === "spot" &&
                        selectedItem?.id === spot.id
                        ? "bg-indigo-600"
              ...
              <div className="space-y-1">
                {trails.map((trail) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setSelectedItem({ type: "trail", id: trail.id })}
                    className={`w-full text-left px-3 py-2 rounded text-sm ${
                      selectedItem?.type === "trail" &&
                        selectedItem?.id === trail.id
                        ? "bg-purple-600"
                        : "bg-gray-200 hover:bg-gray-300"
                    }`}
                  >

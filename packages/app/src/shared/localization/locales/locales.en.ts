const en = {
  common: {
    add: 'Add',
    ok: 'OK',
    cancel: 'Cancel',
    confirm: 'Confirm',
    save: 'Save',
    submit: 'Submit',
    back: 'Back',
    next: 'Next',
    loading: 'Loading...',
    error: 'Error',
    close: 'Close',
    delete: 'Remove',
    edit: 'Edit',
    view: 'View',
    dismiss: 'Dismiss',
    enabled: 'Enabled',
    disabled: 'Disabled',
    run: 'Run',
    more: 'More',
    select: 'Select',
    remove: 'Remove',
    confirmRemove: 'Are you sure you want to remove this?',
    status: 'Status',
    created: 'Created',
  },
  errors: {
    generic: 'An error has occurred.',
    notFound: 'Not found.',
    serverError: 'Server error.',
  },
  validation: {
    nameMinLength: 'Name must be at least 2 characters',
    contentBlockEmpty: 'All content blocks must be filled in',
  },
  navigation: {
    home: 'Home',
    settings: 'Settings',
    profile: 'Profile',
    trails: 'Trails',
    feed: 'Spots',
    map: 'Map',
    about: 'ferthe',
    account: 'Account',
    socials: 'Socials',
  },
  trails: {
    yourTrails: 'Trails',
    startTrail: 'Start Trail',
    everyJourney: 'Every journey begins with a single step',
    createTrail: 'Create a Trail',
    titlePrompt: "What's the title of your journey?",
    nameTrail: 'Name your trail',
    curious: 'What are you curious about?',
    describeClue: 'Describe your clue',
    linkClue: 'Link a clue',
    recurrence: 'Recurrence',
    chooseFrequency: 'Choose how often a new clue appears',
    scheduler: 'Scheduler',
    selectDays: 'Select Days',
    dayOfMonth: 'Day of Month',
    selectDate: 'Select Date',
    time: 'Time',
    preview: 'Preview',
    titleLabel: 'Title',
    descriptionLabel: 'Description',
    tagsLabel: 'Tags',
    clueLabel: 'Clue',
    linksLabel: 'Links',
    recurrenceLabel: 'Recurrence',
    notificationsLabel: 'Notifications',
    noDescription: 'No description available',
    overview: 'Overview',
    description: 'Description',
    spots: 'Spots',
    unknownSpots: 'Unknown Spots',
    spot: 'spot',
    toDiscover: 'to discover',
    allSpotsDiscovered: 'All spots discovered!',
    rating: 'Trail Rating',
    stats: {
      name: 'Your Stats',
      progress: 'Progress',
      status: 'Status',
      rank: 'Rank',
      of: 'of',
      averageTime: 'Average Time',
      completionStatus: {
        not_started: 'Not Started',
        in_progress: 'In Progress',
        completed: 'Completed',
      },
    },
  },
  recurrence: {
    none: 'None',
    daily: 'Daily',
    weekly: 'Weekly',
    monthly: 'Monthly',
    single: 'Single-Time Event',
    noRecurrence: 'No recurrence',
    everyDay: 'Every day at',
    weeklyOn: 'Weekly on',
    monthlyOnDay: 'Monthly on day',
    on: 'On',
    at: 'at',
    noDaysSelected: 'no days selected',
  },
  settings: {
    customize: 'Customize your Journey',
    yourSettings: 'Your Settings',
    forestIs: 'The forest is...',
    light: 'Light',
    dark: 'Dark',
    language: 'Language',
    chooseLanguage: 'Choose your language',
  },
  about: {
    followYourTrail: 'Follow the trail...',
    fertheWebsite: 'ferthe.de',
    storyIntro: 'Somewhere in Münster, in the heart of Europe, a fox picks up a trail. Not in haste, but with intent. Focused. Curious. Always moving forward.',
    aboutText:
      'Ferthe follows that spirit – helping you stay on top of things in a world overflowing with information. Rooted in European values: transparency, self-determination, and digital sovereignty. Thoughtfully built in Münster – where history, progress, and quality of life meet.',
    latestPosts: 'Latest Posts',
  },
  notifications: {
    enable: 'Enable notifications',
    newMessage: 'You have a new clue!',
  },
  auth: {
    welcomeToFerthe: 'Welcome to Ferthe',
    enterPhoneNumber: 'Enter your phone number to get started',
    orCreateLocal: 'or create a local account',
    skipPhoneNumber: 'Skip phone number',
    createLocalAccount: 'Create local account',
    phoneNumberPlaceholder: 'Phone number',
    sendSms: 'Send Code',
    localAccountNotice: 'You can add your phone number later to unlock all features',
    upgradeAccountLater: 'You can upgrade your account anytime in settings',
    upgradeAccount: 'Upgrade Account',
    verifyPhoneNumber: 'Verify Phone Number',
  },
  account: {
    myAccount: 'Account',
    accountStatus: 'Account Status',
    localAccount: 'Local Account',
    phoneAccount: 'Phone Account',
    accountType: 'Account Type',
    featureAccess: 'Feature Access',
    fullAccess: 'Full Access',
    limitedAccess: 'Limited Access',
    upgradeToUnlock: 'Upgrade to unlock all features',
    login: 'Login',
    logout: 'Logout',
    upgradeAccount: 'Upgrade Account',
    phoneVerification: 'Phone Verification',
    accountId: 'Account ID',
    loggedInAs: 'Logged in as',
    notLoggedIn: 'Not logged in',
    notVerified: 'Not verified',
    loginToSync: 'Login to sync your data across devices',
    localAccountDescription: 'Your data is stored locally on this device',
    phoneAccountDescription: 'Your data is synced across all your devices',
    displayName: 'Display Name',
    setDisplayName: 'Set Display Name',
    displayNamePlaceholder: 'Enter your name',
    save: 'Save',
    displayNameUpdated: 'Display name updated',
    skip: 'skip',
    yourPhoneNumber: 'Your phone number',
    sendCode: 'Send Code',
    verificationCode: 'Verification Code',
    yourVerificationCode: 'Your verification code',
    verifyCode: 'Verify Code',
    description: 'Description',
    upgradeNow: 'Upgrade now',
    selectImage: 'Select Image',
    addDescription: 'Add a description...',
    uploadAvatar: 'Upload Avatar',
    avatarUploadSuccess: 'Avatar updated successfully',
    avatarUploadFailed: 'Failed to update avatar',
    notSet: 'Not set',
    profile: 'Profile',
    spots: 'Spots',
    trails: 'Trails',
    ratings: 'Ratings',
    deleteAccount: 'Delete Account',
    deleteAccountConfirm: 'Are you sure? This will permanently delete your account and all your data.',
    logoutConfirm: 'Are you sure you want to logout?',
    logoutConfirmUnverified: 'Are you sure you want to logout? Since your account is not verified, all your data on this device will be permanently removed.',
  },
  community: {
    communities: 'Communities',
    yourCommunities: 'Your communities',
    greeting: 'Hi',
    defaultName: 'Fox',
    createNewCommunity: 'Create New Community',
    editCommunity: 'Edit Community',
    joinCommunity: 'Join Community',
    leave: 'Leave',
    remove: 'Remove',
    confirmRemove: 'Remove this community?',
    communityName: 'Community Name',
    yourCommunityName: 'Your Community Name',
    selectTrail: 'Select Trail',
    create: 'Create',
    joinWithInviteCode: 'Join with Invite Code',
    yourCode: 'Your Code',
    join: 'Join',
    shareDiscovery: 'Share Discovery',
    selectCommunity: 'Select Community',
    shareSuccess: 'Discovery shared successfully',
    shareFailed: 'Failed to share discovery',
    alreadyShared: 'Discovery already shared in this community',
    discoveryTooOld: 'Can only share discoveries from the last 24 hours',
    notInCommunityTrail: 'This spot is not part of the community trail',
    unshareDiscovery: 'Unshare Discovery',
    removeDiscoveryConfirm: 'Remove this discovery from the community?',
    unshare: 'Unshare',
    placeholder: 'C',
    noDiscoveriesYet: 'No discoveries shared yet',
    shareFromProfile: 'Share discoveries from your profile to see them here',
    overview: 'Overview',
    members: 'Members',
    inviteCode: 'Invite Code',
    created: 'Created',
  },
  discovery: {
    discoveries: 'Your Discoveries',
    noDiscoveries: 'No discoveries yet',
    discoveriesSubtitle: 'All your Discoveries',
    editYourDiscovery: 'Edit your discovery',
    documentYourDiscovery: 'Document your discovery',
    pickingImage: 'Picking image...',
    pickImageFromDevice: 'Pick Image from Device',
    shareYourStory: 'Share your story',
    shareYourStoryPlaceholder: 'Share your story...',
    visibilityLabel: 'Visibility',
    visibilityPrivate: 'Private',
    visibilityPublic: 'Public',
    visibilityPrivateHelp: 'Only visible to you',
    visibilityPublicHelp: 'Visible in your public profile',
    cancel: 'Cancel',
    processing: 'Processing...',
    saving: 'Saving...',
    update: 'Update',
    save: 'Save',
    share: 'Share',
    addNote: 'Share your story',
    discovered: 'Discovered',
    deleteImage: 'Delete Image',
    deleteImageConfirm: 'Are you sure you want to delete this image?',
    stats: {
      name: 'Stats',
      rank: 'Discovery Rank',
      of: 'of',
      trailPosition: 'Trail Position',
      timeSinceLast: 'Time Since Last',
      distanceFromLast: 'Distance From Last',
    },
    rating: {
      rateDiscovery: 'Rate this discovery',
      averageRating: 'Average rating',
      yourRating: 'Your rating',
    },
  },
  map: {
    selectTrail: 'Select Trail',
    outsideTrail: 'Outside the trail',
    distanceAway: 'away',
  },
  spotCreation: {
    title: 'Create Spot',
    editTitle: 'Edit Spot',
    stepContent: 'Content',
    stepOptions: 'Options',
    stepConsent: 'Confirm',
    name: 'Name',
    namePlaceholder: 'Give your spot a name',
    description: 'Description',
    descriptionPlaceholder: 'Describe this spot...',
    addPhoto: 'Add Photo',
    changePhoto: 'Change Photo',
    takePhoto: 'Take Photo',
    chooseFromGallery: 'Choose from Gallery',
    visibility: 'Visibility',
    visibilityPreview: 'Preview',
    visibilityPreviewDescription: 'Shows clue on map',
    visibilityHidden: 'Hidden',
    visibilityHiddenDescription: 'Not visible until discovered',
    visibilityPrivate: 'Private',
    visibilityPrivateDescription: 'Only visible to you',
    visibilityPublic: 'Public',
    visibilityPublicDescription: 'Always visible to everyone',
    assignToTrails: 'Assign to Trails',
    noTrailsSelected: 'Standalone spot (no trail)',
    consentTitle: 'Before you create',
    consentText: '• I have permission to create and share this spot.\n• I will not use copyrighted content without permission.\n• I will not share inappropriate or offensive content.\n• I will respect the privacy of others and not share personal information without consent.\n• I understand that I am responsible for the content I create and share.',
    consentAgree: 'I agree to these terms.',
    createSpot: 'Create Spot',
    creating: 'Creating...',
    created: 'Spot created!',
    error: 'Failed to create spot',
    locationRequired: 'Location is required to create a spot',
    yourSpots: 'Your Spots',
    noSpots: 'No spots yet',
    yourSpotsSubtitle: 'All your Spots',
  },
  spot: {
    rating: 'Spot Rating',
    created: 'Created',
    creator: 'Creator',
    you: 'You',
  },
  stumble: {
    title: 'Stumble Mode',
    subtitle: 'Find interesting places near you to document',
    activating: 'Finding interesting places...',
    noSuggestions: 'No suggestions found nearby. Try different preferences.',
    preferencesTitle: 'What are you interested in?',
    start: 'Start Stumbling',
    stop: 'Stop Stumbling',
    createSpot: 'Create Spot here',
    categories: {
      historical: 'Historical',
      cafe: 'Café',
      art: 'Art',
      architecture: 'Architecture',
      nature: 'Nature',
      street_art: 'Street Art',
    },
  },
  contentBlocks: {
    addText: 'Text',
    addQuote: 'Quote',
    addImage: 'Image',
    addLink: 'Link',
    text: 'Text',
    textPlaceholder: 'Enter text...',
    quote: 'Quote',
    quotePlaceholder: 'Enter quote...',
    quoteAuthor: 'Author',
    quoteAuthorPlaceholder: 'Who said this?',
    image: 'Image',
    changeImage: 'Change Image',
    imageCaption: 'Caption',
    imageCaptionPlaceholder: 'Describe this image...',
    link: 'URL',
    linkPlaceholder: 'https://...',
    linkLabel: 'Label',
    linkLabelPlaceholder: 'Link text...',
    contentBlocks: 'Content',
  },
  feedback: {
    title: 'Feedback',
    namePlaceholder: 'Your name',
    emailPlaceholder: 'your@email.com',
    typeLabel: 'Type',
    typeGeneral: 'General',
    typeBug: 'Bug',
    typeReport: 'Report',
    typeFeedback: 'Feedback',
    messagePlaceholder: 'Tell us what you think...',
    messageRequired: 'Message is required',
    submit: 'Send Feedback',
    submitting: 'Sending...',
    successMessage: 'Thank you for your feedback!',
    errorMessage: 'Failed to submit feedback',
  },
}

export type LocalizationSet = typeof en
export default en
